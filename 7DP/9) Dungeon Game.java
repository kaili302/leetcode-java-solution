/*
The demons had captured the princess (P) and imprisoned her in the bottom-right corner of a dungeon. 
The dungeon consists of M x N rooms laid out in a 2D grid. 
Our valiant knight (K) was initially positioned in the top-left room and must fight his way through the dungeon 
to rescue the princess.
The knight has an initial health point represented by a positive integer. 
If at any point his health point drops to 0 or below, he dies immediately.
Some of the rooms are guarded by demons, so the knight loses health (negative integers) upon entering these rooms; 
other rooms are either empty (0's) or contain magic orbs that increase the knight's health (positive integers).
In order to reach the princess as quickly as possible, 
the knight decides to move only rightward or downward in each step.

Write a function to determine the knight's minimum initial 
health so that he is able to rescue the princess.
For example, given the dungeon below, the initial health of the knight must be at least 7 
if he follows the optimal path RIGHT-> RIGHT -> DOWN -> DOWN.
-2 (K)	-3	3
-5	-10	1
10	30	-5 (P)
*/
public class Solution {
    static int minHealth[][], w, h;
    public static int calculateMinimumHP(int[][] A){
        h=A.length; 
        w=A[0].length;
        minHealth=new int[h][w];
        dp(A, 0 , 0);
        return minHealth[0][0]+1;
    }
    public static int dp(int[][] A, int i, int j){
        if (minHealth[i][j]!=0) return minHealth[i][j];
        int next=0;
        if (i==h-1 && j==w-1) next=0;
        else if (i==h-1) next=dp(A, i, j+1);
        else if (j==w-1) next=dp(A, i+1, j);
        else next=Math.min(dp(A, i+1, j), dp(A, i, j+1));
        minHealth[i][j]=(A[i][j]-next)>=0?0: next-A[i][j];
        return minHealth[i][j];
    }
    
    

}    
    //non recursion
public class Solution {
    public static int calculateMinimumHP(int[][] A){
        int minHealth[][], w, h, next;
        h=A.length; 
        w=A[0].length;
        minHealth=new int[h][w];
        for (int i=h-1; i>=0; i--){
            for(int j=w-1; j>=0; j--){
                if (i==h-1 && j==w-1) next=0; 
                else if (i==h-1) next=minHealth[i][j+1];
                else if (j==w-1) next=minHealth[i+1][j];
                else next=Math.min(minHealth[i][j+1], next=minHealth[i+1][j]);
                minHealth[i][j]=(A[i][j]-next)>=0?0: next-A[i][j];
            }
        }
        return minHealth[0][0]+1;
    }
}